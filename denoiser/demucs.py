...
class Demucs(nn.Module):
    ...
    def forward(self, mix):
        ...
        if self.normalize:
            ...
            std = 1
        length = mix.shape[-1]
        x = mix #torch.Size([16, 1, 64085])
        #print('mix shape: ', x.size()) 
        x = F.pad(x, (0, self.valid_length(length) - length))
        #print('mix pad shape: ', x.size())
        if self.resample == 2:
            x = upsample2(x)
            ...
            x = upsample2(x)
        skips = []
        #print('mix upsample shape: ', x.size()) torch.Size([16, 1, 256340])
        for encode in self.encoder:
            x = encode(x)
            skips.append(x)
        #print('x encode shape: ', x.size()) torch.Size([16, 768, 249])
        x = x.permute(2, 0, 1)
        x, _ = self.lstm(x)
        x = x.permute(1, 2, 0)
        #print('x lstm shape: ', x.size()) torch.Size([16, 768, 249])
        for decode in self.decoder:
            skip = skips.pop(-1)
            x = x + skip[..., :x.shape[-1]]
            x = decode(x)
        #print('x decode shape: ', x.size()) torch.Size([16, 1, 256340])
        if self.resample == 2:
            x = downsample2(x)
        elif self.resample == 4:
            x = downsample2(x)
            x = downsample2(x)
        #print('x downsample shape: ', x.size()) torch.Size([16, 1, 64085])
        x = x[..., :length]
        return std * x
